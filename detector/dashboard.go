...
func NewDashboard(blocker *Blocker, window *SlidingWindow, baseline *BaselineEngine, port int) *Dashboard {
	return &Dashboard{
		blocker:   blocker,
		window:    window,
		baseline:  baseline,
...
<div id="warmup-banner" class="warmup-banner" style="display:none">
  ⏳ Baseline warming up — anomaly detection is paused until enough traffic data is collected (2 min).
</div>
